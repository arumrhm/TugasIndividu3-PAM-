...
import {
    JADWAL, 
    MASKAPAI, 
    BANDARA} from '../database/Database';
                            ...
                            <View style={searchResultStyle.columnContainer}>
                                    <View style={searchResultStyle.rowContainer}>
                                        <Text style={searchResultStyle.bandara}>
                                            {BANDARA.find(subItem => subItem.bandara_kode === item.bandara_kode_keberangkatan).bandara_nama}
                                        ...
                                        >
                                            {BANDARA.find(subItem => subItem.bandara_kode === item.bandara_kode_tujuan).bandara_nama}
                                        </Text>
                                    </View>
                            <View style={searchResultStyle.rowContainer}>
        ...
        if (asal !== '' || tujuan !== '' || tanggal !== '') {
            ...
            if (departureId != "" && arrivalId != "") {
                ...
                const searchResult = JADWAL.filter(
                    item => item.bandara_kode_keberangkatan.toLowerCase().replace(/\s/g, '') 
                    === depId.toLowerCase().replace(/\s/g, '') && item.bandara_kode_tujuan.toLowerCase().replace(/\s/g, '') 
        ...
        <View style={searchResultStyle.resultContainer}>
            <RenderData/>
                <Text style={searchResultStyle.copyRight}>
                    Copyright Arum Rahmawati-120140085
                </Text>
        </View>
    );
};
